...
def evaluate(num_votes):
    ...
    with tf.device('/gpu:'+str(GPU_INDEX)):
        ...
        # simple model
        pred, end_points = MODEL.get_model(pointclouds_pl, is_training_pl, ROTATION_NUMBER)
...
def eval_one_epoch(sess, ops, num_votes=1, topk=1):
    ...
    for fn in range(len(TEST_FILES)):
        ...
        print(file_size)
        if PROFILE_DEBUG : num_batches = 15
        for batch_idx in range(num_batches):
            ...
            batch_pred_sum = np.zeros((cur_batch_size*ROTATION_NUMBER, ROTATION_NUMBER)) # score for classes
            batch_pred_classes = np.zeros((cur_batch_size*ROTATION_NUMBER, ROTATION_NUMBER)) # 0/1 for classes
